...
import { aboutThisRaise, eventAbout, raiseWindows, touchpointsFor, type Touchpoint } from '@/modules/meetings';
...
export async function ContactHistory({ entityId, vehicleIds }: {
  ...
  // About a raise, which one nobody has said yet (N81): counted for none until tagged.
  const unclear = own.filter((t) => !aboutAny.includes(t) && eventAbout(t, [...windows.values()]).kind === 'unclear');
  const other = countContact(own.filter((t) => !aboutAny.includes(t) && !unclear.includes(t)), now);
    ...
    <div className="card">
      ...
      <div className="cbody">
        ...
        <div className="fact">
          ...
          <span>{aboutAny.filter((t) => t.on && t.on.getTime() <= now).length}, each tagged with a vehicle — it names it, or Claude or a person said so — and inside its window</span>
        </div>
        {unclear.length > 0 && (
          <div className="fact">
            <span>Vehicle unclear</span>
            <span>{unclear.length} about a raise without saying which — counted for none until someone tags it on the LP&rsquo;s timeline</span>
          </div>
        )}
        {byVehicle.map(({ w, n }) => (
      ...
      <p className="cover">
        ...
        pages count only contact tagged with that vehicle, inside its window; the rest is kept here
        as history for research and strategy, and counted on no pipeline.
      </p>
    </div>
  );
}
