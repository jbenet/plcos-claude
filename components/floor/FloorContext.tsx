...
export interface FloorFilter {
  ...
  signal: 'all' | 'hot' | 'quiet' | 'blocked' | 'urgent' | 'stalled' | 'unsized' | 'wired';
  stage: string | 'all';
}

export const EMPTY_FILTER: FloorFilter = {
  find: '', owner: 'everyone', signal: 'all', stage: 'all',
};
...
export function matches(item: FloorItem, f: FloorFilter): boolean {
  ...
  if (f.stage !== 'all' && (item.rung ?? 'none') !== f.stage) return false;
  switch (f.signal) {
  ...
  if (f.find.trim()) {
    ...
    const hay = [
      ...
      item.blocked ?? '', item.urgent ?? '', item.rung ?? '',
    ].join(' ').toLowerCase();
