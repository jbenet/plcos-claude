...
import {
  REASONS, STAGES, type OutcomeReason, type PursuitOutcome, type PursuitStage,
 ...
 * fields say where an entry is, tried in order; what each of their values means as our stage,
 * outcome and reason; and which fields hold the amounts, the owner, the introducer and a
 * do-not-contact mark.
 *
...
const OUTCOMES: PursuitOutcome[] = ['open', 'paused', 'passed', 'lost'];
...
/** What a status seems to mean. A proposal for a person to review, never a fact. */
export function proposeValue(value: string): ValueMap | null {
  ...
  if (/on hold|paused/.test(v)) {
    const inner = proposeValue(v.replace(/on hold|paused/, '').replace(/^[\s\-–—:]+/, ''));
    return { stage: inner?.stage ?? null, outcome: 'paused' };
  }
  if (/\blost\b/.test(v)) return { stage: /no response/.test(v) ? 'contacted' : null, outcome: 'lost', reason: reasonOf(v) ?? 'other' };
  if (/pass|declin|not interested/.test(v)) return { stage: null, outcome: 'passed', reason: reasonOf(v) ?? 'other' };
  if (/wired|funded|cash|received/.test(v)) return { stage: 'funded' };
  if (/signed|countersign|closed won|\bcommitted\b/.test(v)) return { stage: 'signed' };
  if (/soft|verbal|circle/.test(v)) return { stage: 'soft_commit' };
  if (/docs out|documents sent|docs sent/.test(v)) return { stage: 'docs_out' };
  if (/diligence|\bdd\b/.test(v)) return { stage: 'diligence' };
  if (/(2\+|two|second|follow).*meeting|meetings/.test(v)) return { stage: 'follow_up' };
  if (/meeting|call complete|met\b/.test(v)) return { stage: 'first_meeting' };
  if (/schedul/.test(v)) return { stage: 'scheduling' };
  if (/respond|interested|discussion|replied/.test(v)) return { stage: 'responded' };
  // "In progress" says we are working it, not that they answered.
  if (/contacted|reached out|intro made|introduced|sent|in progress/.test(v)) return { stage: 'contacted' };
  if (/target|enriched|qualified/.test(v)) return { stage: 'targeted' };
  if (/research|^new$|not started|to do|prospect/.test(v)) return { stage: 'research' };
  return null;
}
...
function block(l: ListInventory, m: ListMapping, counts: Map<string, number>): string {
  ...
  out.push(`      // Where each entry is: the first of these fields with a value wins. For each value, our`);
  out.push(`      // stage (${STAGES.map((s) => s.id).join(', ')}),`);
  out.push(`      // outcome (open, paused, passed, lost) and reason (${REASONS.join(', ')}). null: unplaced.`);
  out.push(`      "stage": [`);
  m.stage.forEach((src, i) => {
    out.push(`        {`);
    ...
    out.push(`          }`);
    out.push(`        }${i < m.stage.length - 1 ? ',' : ''}`);
  });
  out.push(`      ],`);
  out.push(`      // The commitment: soft, always, until the close room has a countersignature. A "signed"`);
  out.push(`      // stage marks it as ready to harden; it does not harden it (rule 1).`);
  ...
  const blocks = lists.map((l) => {
    ...
    const m: ListMapping = kept
      ? {
          ...
          // keeps what a person wrote, null included. A stage field that appeared is added last.
          stage: [
            ...kept.stage.map((src) => {
              const fresh = proposed.stage.find((p) => p.field === src.field);
              return { field: src.field, values: { ...(fresh?.values ?? {}), ...src.values } };
            }),
            ...proposed.stage.filter((p) => !kept.stage.some((k) => k.field === p.field)),
          ],
...
export async function readMapping(inv?: Inventory, path = MAPPING_PATH): Promise<MappingReport> {
  ...
  for (const [name, m] of Object.entries(lists)) {
    ...
    for (const src of (Array.isArray(m.stage) ? m.stage : []) as Array<{ field?: unknown; values?: Record<string, unknown> }>) {
      const f = typeof src.field === 'string' ? src.field : null;
      if (!f) { report.problems.push(`"${name}".stage: each entry needs a "field".`); continue; }
      if (known && !fields.has(f)) report.problems.push(`"${name}".stage: no field called "${f}" on that list.`);
      ...
      for (const [value, vm] of Object.entries(src.values ?? {})) {
        ...
        const x = vm as Record<string, unknown>;
        const bad: string[] = [];
        if (x.stage !== undefined && x.stage !== null && !STAGE_IDS.has(x.stage as PursuitStage)) bad.push(`stage "${String(x.stage)}"`);
        if (x.outcome !== undefined && !OUTCOMES.includes(x.outcome as PursuitOutcome)) bad.push(`outcome "${String(x.outcome)}"`);
        ...
        values[value] = {
          ...(x.stage !== undefined ? { stage: x.stage as PursuitStage | null } : {}),
          ...(x.outcome !== undefined ? { outcome: x.outcome as PursuitOutcome } : {}),
          ...(x.reason !== undefined ? { reason: x.reason as OutcomeReason } : {}),
          ...(x.skip === true ? { skip: true } : {}),
        };
        report.mapped++;
      }
      stage.push({ field: f, values });
    }
...
/**
 * Where one entry is, under a list's mapping: the first stage field whose value has a meaning.
 * If none has, the first value found is still returned — unplaced — so the page can say which
 * word needs one, rather than reporting the entry as blank.
 */
export function placeEntry(m: ListMapping, valueOf: (field: string) => string | null): {
  ...
  for (const src of m.stage) {
    const said = valueOf(src.field);
    if (said === null) continue;
    const map = src.values[said] ?? null;
    if (map) return { said, map, field: src.field };
    first ??= { said, field: src.field };
  }
  return first ? { ...first, map: null } : { said: null, map: null, field: null };
}
